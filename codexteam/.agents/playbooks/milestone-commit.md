...
4. Have the read-only Git Steward return one exact `commit-plan` JSON draft with staging paths, any explicit index-only `untrack_paths`, exclusions, commit message, and PR summary.
