...
Machine-readable schema: `schemas/role-policy-v1.json`. Canonical TOML manifests live under `roles/` and reject unknown fields. The optional `mcp_servers` array is a role-specific allowlist; omission means no MCP access and remains digest-compatible with older v1 snapshots. The optional `mcp_tools` object narrows a listed server to a non-empty tool subset and rejects unknown tools for locally owned servers. The launcher pins the normalized policy plus digest per attempt, explicitly disables configured servers outside that allowlist, applies pinned `enabled_tools` overrides, and reports allowed and effective servers and tool subsets plus missing server names. For a new non-Leader attempt using `codexteam-context`, it also derives and pins `mcp_context_project` from the exact direct-child workspace, then starts a project-bound server whose tool schemas omit `project`. Lead remains unbound, and legacy continuations without the field remain unbound. Policy defaults may change for a future draft but cannot change an existing continuation.
