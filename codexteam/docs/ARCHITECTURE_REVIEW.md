...
- `roles/`: strict role-policy-v1 manifests for seven accountable identities.
...
- Architects own requirement-traceable system and repository design but neither implementation nor self-approval. Local Git Steward model turns are read-only and run only at named boundaries.
