...
def test_project_guidance_sync_upgrades_legacy_native_agent_marker(tmp_path: Path):
    plan = initialize_project("Example", "Goal", root=tmp_path, project_id="example")
    target = (
        plan.project_dir
        / ".codexteam"
        / "native-agents"
        / "codexteam_developer.toml"
    )
    target.write_text(
    ...
    changes = sync_project_guidance(plan.project_dir, apply=True)

    assert "update .codexteam/native-agents/codexteam_developer.toml" in changes
