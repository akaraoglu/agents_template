...
def _draft_args(workspace: Path, profile: str | None = None):
    arguments = [
        "--phase",
        "draft",
        "--team",
...
def test_feature_planner_manifest_has_advisory_only_boundary():
    ...
    assert policy.default_profile == "gpt54-mini"
...
def test_spawn_uses_cloud_default_and_accepts_explicit_local_override(
    ...
    default_request = spawn.prepare_request(_draft_args(default_workspace))
    ...
    local_request = spawn.prepare_request(
        _draft_args(local_workspace, "qwen36-27b")
    )
