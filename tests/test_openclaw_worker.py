...
from AgenticTeam.scripts.openclaw_worker import (
    RESULT_BEGIN,
    RESULT_END,
    OpenClawWorkerRunner,
    extract_marked_json,
)
...
def project_event_file(tmp_path, monkeypatch):
    monkeypatch.setenv("TEAM_EVENT_FILE", str(tmp_path / ".openclaw" / "events.jsonl"))
    clear_events()
    yield
    clear_events()


def make_runner(tmp_path: Path) -> OpenClawWorkerRunner:
    task_pack = TaskPack(
        project_id="p1",
        task_id="T001",
    ...
    return OpenClawWorkerRunner(task_pack, lease, actor="morpheus", timeout_seconds=30)
