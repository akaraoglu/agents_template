...
class WorkerRuntimeTests(unittest.TestCase):
    ...
    def test_morpheus_dispatch_sends_task_packet_without_agent_prepare(self, run_mock: mock.Mock) -> None:
        ...
        self.assertIn("ACTION_CATALOG_BEGIN", task_packet)
        self.assertIn("write_draft_file: write each required project-relative artifact path under DRAFT_WRITE_ROOT.", task_packet)
        self.assertIn("write_manifest: write JSON to MANIFEST_WRITE_FILE with artifacts and test_command.", task_packet)
        self.assertIn("python_claw: optional diagnostics only; never DONE evidence.", task_packet)
        self.assertIn("morpheus_report: run REPORT_COMMAND exactly with RUN_DIR", task_packet)
        self.assertIn("morpheus_block: run BLOCK_COMMAND exactly with RUN_DIR", task_packet)
        self.assertIn("ACTION_CATALOG_END", task_packet)
        self.assertLess(task_packet.index("ACTION_CATALOG_BEGIN"), task_packet.index("REPORT_COMMAND="))
        self.assertLess(task_packet.index("ACTION_CATALOG_END"), task_packet.index("REPORT_COMMAND="))
        ...
        packet_lines = task_packet.splitlines()
        report_line = next(line for line in packet_lines if line.startswith("REPORT_COMMAND="))
        block_line = next(line for line in packet_lines if line.startswith("BLOCK_COMMAND="))
        run_dir = str(Path(state["task_packet_file"]).parent)
        self.assertIn(run_dir, report_line)
        self.assertIn(run_dir, block_line)
        self.assertNotIn(str(state["draft_write_root"]), report_line)
        self.assertNotIn(str(state["draft_write_root"]), block_line)
        self.assertNotIn(str(state["manifest_write_file"]), report_line)
        self.assertNotIn(str(state["manifest_write_file"]), block_line)
