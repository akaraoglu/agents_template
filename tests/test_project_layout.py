...
from AgenticTeam.scripts.project_layout import (
...
def test_ensure_layout_preserves_existing_project_process_shape(tmp_path):
    ...
    missing = validate_project_layout(tmp_path, require_runtime_files=False)
    assert missing == []


def test_write_planning_files_creates_plan_and_task_docs(tmp_path):
    ensure_project_layout(tmp_path, project_id="demo", title="Demo", goal="Goal")
    write_planning_files(
        tmp_path,
        [
...
def test_create_project_creates_persistent_project_layout(tmp_path):
    result = subprocess.run(
        [
            sys.executable,
            "AgenticTeam/scripts/create_project.py",
            "--project-root",
            str(tmp_path),
            "--project-id",
            "demo-team-project",
    ...
    project_dir = tmp_path / "demo-team-project"
