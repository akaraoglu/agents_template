...
from AgenticTeam.scripts.contracts import TaskPack, Lease
...
from AgenticTeam.scripts.tools import TeamToolError
...
def test_morpheus_prompt_compilation(tmp_path):
    ...
    runner = DirectWorkerRunner(tp, lease)
...
def test_morpheus_rejects_protected_project_paths(tmp_path):
    ...
    with pytest.raises(TeamToolError, match="path_protected"):
...
def test_morpheus_rejects_paths_outside_writable_scope(tmp_path):
    ...
    with pytest.raises(TeamToolError, match="path_not_writable"):
...
def test_morpheus_done_requires_expected_artifacts(tmp_path):
    ...
    with pytest.raises(TeamToolError, match="missing_expected_artifacts"):
