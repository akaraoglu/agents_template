...
def test_run_team_dry_run_creates_and_completes_project(tmp_path):
    result = subprocess.run(
        [
            sys.executable,
            "AgenticTeam/scripts/run_team.py",
            "--dry-run",
            "--project-root",
            str(tmp_path),
            "--project-id",
    ...
    project_dir = tmp_path / "tiny-markdown-counter"
    assert f"PROJECT_CREATED={project_dir}" in result.stdout
    assert "TEAM_RESULT=DONE" in result.stdout
...
def test_run_worker_task_final_no_result_blocks_project(tmp_path):
    from AgenticTeam.scripts.run_team import run_worker_task
    ...
    with mock.patch(
        "AgenticTeam.scripts.run_team.OpenClawWorkerRunner.run",
