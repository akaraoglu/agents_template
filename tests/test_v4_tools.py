...
def test_read_and_discovery(workspace_with_lease):
    ...
    tools = V4Tools(str(project_root))
    ...
    with pytest.raises(V4ToolError, match="path_outside_workspace"):
...
def test_mutation_failures(workspace_with_lease):
    ...
    with pytest.raises(V4ToolError, match="stale_lease"):
    ...
    with pytest.raises(V4ToolError, match="content_too_large"):
...
def test_work_and_oracle_submission(workspace_with_lease):
    ...
    with pytest.raises(V4ToolError, match="invalid_payload"):
