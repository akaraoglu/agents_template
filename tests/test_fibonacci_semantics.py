...
from AgenticTeam.scripts.run_e2e_fibonacci_test import (
    latest_oracle_report,
    latest_oracle_repair_task_id,
    validate_fibonacci_visualizer_project,
)
from AgenticTeam.scripts.run_team import validate_fibonacci_fixture_artifacts
...
def test_latest_oracle_repair_task_id_returns_newest_repair_task():
    events = [
        Event(event_type="task_planned", payload={"task_id": "T001", "title": "Implement thing"}, actor="smith"),
        Event(
...
def test_latest_oracle_report_can_be_scoped_to_current_dispatch():
    old_events = [
        Event(event_type="oracle_failed", payload={"summary": "old failure"}, actor="oracle"),
        Event(event_type="oracle_dispatched", payload={"lease_id": "new"}, actor="smith"),
    ]
