...
from AgenticTeam.scripts.run_e2e_fibonacci_v4_neo_test import (
    build_neo_fibonacci_request,
    validate_neo_e2e_project_structure,
)
from AgenticTeam.scripts.run_v4_project import fibonacci_planned_tasks


def test_neo_fibonacci_request_uses_real_v4_startup_path():
    request = build_neo_fibonacci_request(
        project_id="run-e2e-fibonacci-v4-neo-test",
        title="Fibonacci Tree Visualizer",
        project_root=Path("/tmp/active-projects"),
        project_info="# Project: Fibonacci Tree Visualizer\n",
    )

    assert "Neo, start this as a real V4 project" in request
    assert "run-e2e-fibonacci-v4-neo-test" in request
    assert "/home/alik/workspace/clawspace/bin/run_v4_team.sh" in request
    assert "--background" in request
    assert "--fixture fibonacci_tree_visualizer" in request
    assert "V4_TEAM_STARTED" in request
    assert "V4_TEAM_LOG" in request
