...
from AgenticTeam.scripts.run_e2e_fibonacci_v4_test import validate_fibonacci_visualizer_project
from AgenticTeam.scripts.run_v4_project import DEFAULT_PROJECT_ROOT
from AgenticTeam.scripts.v4_project_layout import project_dir_for


DEFAULT_TITLE = "Fibonacci Tree Visualizer"
NEO_PROJECT_FILE = Path("/home/alik/workspace/clawspace/workspaces/neo/v4_PROJECT.md")
LIVE_V4_COMMAND = "/home/alik/workspace/clawspace/bin/run_v4_team.sh"


def default_project_id() -> str:
    stamp = datetime.datetime.now(datetime.timezone.utc).strftime("%Y%m%d-%H%M%S")
    return f"run-e2e-fibonacci-v4-neo-{stamp}"
...
def build_neo_fibonacci_request(
    *,
    ...
    return f"""Neo, start this as a real V4 project through your normal V4 project intake path.

This is the full-team V4 Fibonacci E2E. Do not run Smith, Morpheus, or Oracle manually. Do not use legacy project scripts, handoff scripts, named-session routing, or subagents.
...
2. Run the V4 team command in background mode, including the known Fibonacci fixture selector:
   `bash {LIVE_V4_COMMAND} --background --project-root {project_root} --project-id "{project_id}" --title "{title}" --project-file {NEO_PROJECT_FILE} --fixture fibonacci_tree_visualizer`
3. Do not wait inside your own turn for Smith, Morpheus, or Oracle to finish; the background runtime owns that loop.
4. Report the printed `V4_TEAM_STARTED`, `PROJECT_ID`, `EXPECTED_PROJECT_PATH`, `V4_TEAM_PID`, and `V4_TEAM_LOG` values.
...
def run_neo_fibonacci_e2e(args: argparse.Namespace) -> int:
    ...
    print("Neo-driven V4 Fibonacci E2E passed.")
    return 0


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the real Neo-intake V4 Fibonacci E2E.")
