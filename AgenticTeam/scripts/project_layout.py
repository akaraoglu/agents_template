...
def normalize_project_id(value: str) -> str:
    ...
    return project_id or "team-project"
...
def validate_project_layout(workspace_root: str | Path, *, require_runtime_files: bool = True) -> list[str]:
    workspace = Path(workspace_root).expanduser().resolve()
    missing: list[str] = []
...
def render_project_md(project_id: str, title: str, goal: str) -> str:
    ...
    return (
        ...
        "## Acceptance Criteria\n"
        "- The project completes through the typed WorkResult and OracleResult flow.\n"
    )
...
def render_plan_md(title: str, planned_tasks: list[dict[str, Any]]) -> str:
    lines = [
        ...
        "",
        "## Strategy",
        "Execute tasks sequentially through the Smith -> Morpheus -> Smith -> Oracle loop.",
        "",
        "## Tasks",
    ]
