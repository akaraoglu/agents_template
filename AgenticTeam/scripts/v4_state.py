...
def project_state_from_events(events: List[EventV4]) -> ProjectStateV4:
...
def write_state_v4(state: ProjectStateV4, workspace_root: str):
...
def render_backlog_v4(state: ProjectStateV4) -> str:
...
def render_current_task_v4(state: ProjectStateV4) -> str:
...
def render_markdown_views(state: ProjectStateV4, workspace_root: str):
    workspace_path = Path(workspace_root)
...
"""
    ...
    backlog_content = render_backlog_v4(state)
    with open(backlog_dir / "BACKLOG.md", "w", encoding="utf-8") as f:
        f.write(backlog_content)
        
    # 3. CURRENT_TASK.md
    current_task_content = render_current_task_v4(state)
    with open(workspace_path / "CURRENT_TASK.md", "w", encoding="utf-8") as f:
        f.write(current_task_content)

def detect_markdown_drift(state: ProjectStateV4, workspace_root: str) -> Dict[str, bool]:
    workspace_path = Path(workspace_root)
    drift = {}
    ...
    if not backlog_file.exists():
        ...
        expected = render_backlog_v4(state).strip()
    ...
    if not ct_file.exists():
        ...
        expected = render_current_task_v4(state).strip()
