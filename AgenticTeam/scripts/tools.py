...
from AgenticTeam.scripts.events import append_event
from AgenticTeam.scripts.leases import validate_lease
...
class TeamToolError(Exception):
    """Base exception for team tool errors."""
...
class TeamTools:
    ...
    def _log_event(self, event_type: str, payload: Dict[str, Any], actor: str = "team_tools"):
        event = Event(
    ...
    def _validate_path(self, relative_path: str) -> Path:
        ...
        if not relative_path.strip():
            raise TeamToolError("path_outside_workspace", "Path cannot be empty")
        ...
        try:
            ...
            raise TeamToolError("path_outside_workspace", f"Path {relative_path} is outside workspace root {self.project_root}")
    ...
    def _check_lease(self, lease_id: str, task_id: str, actor: str, attempt_id: str):
        ...
        if not validate_lease(str(self.project_root), lease_id, task_id, actor, attempt_id):
            raise TeamToolError("stale_lease", "The active lease has expired or is invalid.")
    ...
    def fs_list(self, relative_path: str = ".") -> List[str]:
        ...
        if not target_path.exists() or not target_path.is_dir():
            raise TeamToolError("path_not_allowed", f"Directory {relative_path} does not exist or is not a directory")
    ...
    def fs_read(self, relative_path: str) -> str:
        ...
        if not target_path.exists() or not target_path.is_file():
            raise TeamToolError("path_not_allowed", f"File {relative_path} does not exist or is not a file")
        ...
        try:
            ...
            raise TeamToolError("invalid_payload", f"Failed to read file: {e}")
    ...
    def fs_mkdir(self, relative_path: str, lease_id: str, task_id: str, actor: str, attempt_id: str) -> str:
        ...
        try:
            ...
            raise TeamToolError("invalid_payload", f"Failed to create directory: {e}")
    ...
    def fs_write(self, relative_path: str, content: str, lease_id: str, task_id: str, actor: str, attempt_id: str) -> str:
        ...
        if len(content.encode("utf-8")) > 1024 * 1024:
            raise TeamToolError("content_too_large", "Content exceeds 1MB limit")
        ...
        try:
            ...
            raise TeamToolError("invalid_payload", f"Failed to write file: {e}")
    ...
    def fs_patch(self, relative_path: str, patch_content: str, lease_id: str, task_id: str, actor: str, attempt_id: str) -> str:
        ...
        if not target_path.exists():
            raise TeamToolError("path_not_allowed", f"File does not exist: {relative_path}")
        ...
        try:
            ...
            if not search_str:
                raise TeamToolError("invalid_payload", "Patch must contain SEARCH: block")
            ...
            if search_str in original_content:
                ...
                raise TeamToolError("invalid_payload", "Search string not found in file")
        except TeamToolError:
            raise
        except Exception as e:
            raise TeamToolError("invalid_payload", f"Failed to apply patch: {e}")
    ...
    def tests_run(self, test_path: str, lease_id: str, task_id: str, actor: str, attempt_id: str) -> str:
        ...
        try:
            ...
            raise TeamToolError("invalid_payload", f"Failed to execute tests: {e}")
    ...
    def cmd_run(self, command: str, lease_id: str, task_id: str, actor: str, attempt_id: str) -> str:
        ...
        if not self.cmd_preflight(command):
            raise TeamToolError("path_not_allowed", "Command is not allowlisted")
        ...
        try:
            ...
            raise TeamToolError("invalid_payload", f"Failed to execute command: {e}")

    # --- Work & Oracle Submission Tools ---

    def work_submit(self, work_result: WorkResult, lease_id: str, actor: str) -> str:
        """Submits task work result."""
        self._check_lease(lease_id, work_result.task_id, actor, work_result.attempt_id)
        if not validate_work_result(work_result):
            raise TeamToolError("invalid_payload", "WorkResult validation failed (missing output/evidence/task_id/attempt_id)")
    ...
    def work_block(self, task_id: str, attempt_id: str, reason: str, lease_id: str, actor: str) -> str:
        ...
        self._check_lease(lease_id, task_id, actor, attempt_id)
        if not reason.strip():
            raise TeamToolError("invalid_payload", "Block reason cannot be empty")
    ...
    def work_status(self, task_id: str) -> str:
        ...
        from AgenticTeam.scripts.events import read_events
        events = read_events()
    ...
    def oracle_report(self, oracle_result: OracleResult, lease_id: str, actor: str) -> str:
        ...
        if not validate_oracle_result(oracle_result):
            raise TeamToolError("invalid_payload", "OracleResult must contain evidence paths")
    ...
    def mm_send_message(self, message: str, target: str, actor: str, reply_to: Optional[str] = None) -> str:
        ...
        if not message.strip():
            raise TeamToolError("invalid_payload", "Message body cannot be empty")
        if not target.strip():
            raise TeamToolError("invalid_payload", "Target channel/user cannot be empty")
        ...
        try:
            ...
            raise TeamToolError("mm_delivery_failed", f"Failed to send message: {err}")
    ...
    def mm_read_messages(self, target: str, actor: str, limit: int = 10) -> str:
        ...
        try:
            ...
            err = e.stderr.strip() or e.stdout.strip()
            raise TeamToolError("mm_read_failed", f"Failed to read messages: {err}")
    ...
    def sys_diagnose(self, actor: str) -> str:
        ...
        try:
            ...
            raise TeamToolError("sys_diagnose_failed", f"Failed to run diagnostics: {e}")
    ...
    def sys_run_tests(self, test_path: str, actor: str) -> str:
        ...
        try:
            ...
            raise TeamToolError("sys_run_tests_failed", f"Failed to run diagnostic tests: {e}")
