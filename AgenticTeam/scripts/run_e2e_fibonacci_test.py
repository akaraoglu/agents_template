...
def run_single_e2e(
    ...
    if project_root:
        ...
        project_id = f"run-e2e-fibonacci-{timestamp}-{run_index}"
        workspace_root = project_dir_for(project_root, project_id)
        workspace_root.mkdir(parents=True, exist_ok=False)
        keep_workspace = True
    else:
        project_id = f"fib-team-run-{run_index}"
        temp_dir = tempfile.mkdtemp(prefix=f"fibonacci_team_run_{run_index}_")
        workspace_root = Path(temp_dir)
    print(f"Workspace root: {workspace_root}")
    
    # Isolate team event file
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fibonacci E2E Runner")
    ...
    parser.add_argument("--project-root", help="Create the project under this persistent projects root instead of /tmp")
    args = parser.parse_args()
