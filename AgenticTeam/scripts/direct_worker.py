...
from AgenticTeam.scripts.contracts import TaskPack, Lease, WorkResult
...
from AgenticTeam.scripts.model_runtime import OLLAMA_URL, extract_json, load_ollama_runtime_config

class DirectWorkerRunner:
    def __init__(self, task_pack: TaskPack, lease: Lease, actor: str = "morpheus"):
        self.task_pack = task_pack
        self.lease = lease
        self.actor = actor
        self.tools = TeamTools(task_pack.workspace_root)
...
"""
    ...
    def _execute_tool(self, name: str, params: Dict[str, Any]) -> str:
        ...
        if name == "fs_read":
            ...
            raise TeamToolError("missing_capability", f"Unknown tool: {name}")
