...
from AgenticTeam.scripts.v4_project_layout import ensure_v4_project_layout, render_task_md, write_planning_files
...
def smith_repair_artifact_policy(
    ...
    for evidence_path in oracle_result.evidence_paths or []:
        try:
            normalized = _normalize_artifact_path(evidence_path)
...
def _bind_project_event_file(workspace_root: str) -> None:
    os.environ["V4_EVENT_FILE"] = str(Path(workspace_root).resolve() / ".openclaw" / "events.jsonl")
...
def smith_plan_project(
    ...
    *,
    ...
    """
    _bind_project_event_file(workspace_root)
    ensure_v4_project_layout(workspace_root, project_id=project_id, title=title, goal=goal)
