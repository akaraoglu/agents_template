...
from AgenticTeam.scripts.create_project import DEFAULT_PROJECT_ROOT, fibonacci_planned_tasks, parse_task
...
from AgenticTeam.scripts.leases import release_lease
from AgenticTeam.scripts.openclaw_worker import OpenClawWorkerRunner
from AgenticTeam.scripts.oracle_runtime import OracleRunner
from AgenticTeam.scripts.project_layout import project_dir_for, validate_project_layout
...
from AgenticTeam.scripts.direct_worker import DirectWorkerRunner
    ...
    for relative in expected_artifacts:
        ...
        if relative.endswith(".py"):
            if target.name.startswith("test_"):
                target.write_text("def test_dry_run_placeholder():\n    assert True\n", encoding="utf-8")
            else:
                target.write_text('"""Dry-run implementation placeholder."""\n', encoding="utf-8")
        else:
    ...
    *,
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if worker_backend == "direct":
            runner = DirectWorkerRunner(task_pack, lease, actor="morpheus")
            runner_result = runner.run(max_turns=35)
        else:
            runner = OpenClawWorkerRunner(
                task_pack,
                lease,
                actor="morpheus",
        ...
        state = project_state_from_events(read_events())
...
def run_oracle_cycle(workspace_root: Path, project_id: str, dry_run: bool, fixture: str | None = None) -> str | None:
    ...
    event_count_before_dispatch = len(read_events())
    ...
    runner = OracleRunner(str(workspace_root), lease, actor="oracle")
    runner_result = runner.run(max_turns=25)
    print(f"Oracle result: {runner_result}")

    oracle_event = latest_oracle_report(read_events()[event_count_before_dispatch:])
...
def run_team(args: argparse.Namespace) -> int:
    ...
    missing = validate_project_layout(workspace_root, require_runtime_files=False)
    if missing:
        print(f"ERROR: project layout is incomplete: {missing}", file=sys.stderr)
        return 1

    print(f"PROJECT_CREATED={workspace_root}")
    ...
    print("TEAM_RESULT=DONE")
    print(f"FINAL_PROJECT_PATH={workspace_root}")
    return 0


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Neo -> Smith -> Morpheus -> Oracle project loop.")
    ...
    parser.add_argument("--title", default="Team Project", help="Human-readable project title.")
    ...
    parser.add_argument("--fixture", choices=["fibonacci_tree_visualizer"], help="Use a known fixture plan.")
