...
from AgenticTeam.scripts.contracts import Event, Lease, TaskPack, WorkResult
from AgenticTeam.scripts.events import append_event
from AgenticTeam.scripts.tools import TeamToolError, TeamTools
...
class OpenClawWorkerRunner:
    """Runs a worker task through the configured OpenClaw agent platform."""

    def __init__(self, task_pack: TaskPack, lease: Lease, actor: str = "morpheus", timeout_seconds: int = 600):
        self.task_pack = task_pack
        self.lease = lease
        self.actor = actor
        self.timeout_seconds = timeout_seconds
        self.workspace_root = Path(task_pack.workspace_root).resolve()
        self.tools = TeamTools(str(self.workspace_root))
    ...
    def _agent_command(self) -> list[str]:
        session_key = (
            f"agent:{self.actor}:team:{self.task_pack.project_id}:"
    ...
    def _message(self) -> str:
        ...
        return f"""You are executing a worker task as the real OpenClaw `{self.actor}` agent.
...
The team runtime will validate expected artifacts and write boundaries after your OpenClaw turn returns.
"""
    ...
    def _normalized_relative_path(self, relative_path: str) -> str:
        ...
        if normalized in ("", ".") or normalized.startswith("../") or normalized == "..":
            raise TeamToolError("path_outside_workspace", f"Invalid project-relative path: {relative_path}")
        return normalized
