...
from AgenticTeam.scripts.smith_runtime import smith_plan_project
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Create a persistent project using the existing project-management layout.")
    ...
    os.environ["TEAM_EVENT_FILE"] = str(project_dir / ".openclaw" / "events.jsonl")
    clear_events()
    ...
    missing = validate_project_layout(project_dir, require_runtime_files=False)
    if missing:
        print(f"ERROR: project layout is incomplete: {missing}", file=sys.stderr)
        return 1

    print(f"PROJECT_CREATED={project_dir}")
    print(f"PROJECT_ID={args.project_id}")
    print("NEXT_ACTION=Smith may dispatch the first task through the team conductor.")
    return 0
