...
- builtin local execution for the first project loop path (`agent_smith -> niobe -> architect -> morpheus -> oracle`) and the nested software loop (`morpheus -> planner -> implementer -> tester`) in `runtime/role_executor.py`, `orchestrators/niobe_engine.py`, and `orchestrators/morpheus_engine.py`
